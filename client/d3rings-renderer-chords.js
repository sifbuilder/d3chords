			 ...
			 // Square matrices, asynchronously loaded
				...
				actionsList.forEach(function(d) {
					...
					layout.matrix(entries)		
									...
									// console.log("_____ entries ",JSON.stringify(entries, null, 2))
			        ...
			        // .style("fill", function(d) { return fill(subjectByIndex[d.index].weigh); })			
			        .style("fill", function(d) { return fill(d.index); })			
						...
						.append("title")
			        .text(function(d) { 
									return subjectByIndex[d.index].name + " acts " + format(d.value) + "with weigh " + subjectByIndex[d.index].weigh; });
					...
					groupsElemsNew.append("text")
						...
						.append("textPath")
							...
							.text(function(d) { 
console.log("_________ d.value", d.value)
				 ...
				 // Update group paths
			        ...
			        .style("fill", function(d) { return fill(d.index); })			
									...
									.append("title")
										.text(function(d) {
											return d.source.value.source.name + " predicates " + d.source.value.target.name + format(d.source.value)})
...
console.log("__________d", d)										
					...
					noteElems
						.select("text.note")
							.attr("d", chord)	
										.text(function(d) {
											var r = d.source.value.source.name + " to " + d.source.value.target.name + " " + format(d.source.value) + " " + d.source.value.predicate
											return r
											 })
										.attr("text-anchor", function(d) {
														var angle = (d.source.startAngle + d.source.endAngle) / 2
...
console.log("_________________", d)								
												// return fill(d.source.value.weigh); })
